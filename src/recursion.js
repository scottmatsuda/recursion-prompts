...
var exponent = function(base, exp) {
    ...
    if (exp > 0) {
        ...
        return parseFloat(Number.parseFloat((1 / base)*  exponent(base, exp + 1)).toFixed(5));
    }
};
...
var reverse = function(string) {
    let rev = '';
    rev.concat(string[string.length - 1]);
...
var reverseArr = function(array) {
    ...
    reverse.push(array.pop());
        if (array.length === 0) {
            ...
            return reverse.concat(reverseArr(array));
        }
};
...
var rMap = function(array, callback) {
    ...
    if (array.length < 1) {
        ...
        return map.concat(rMap(array.slice(1), callback));
    }
};
...
var alternateSign = function(array) {
    ...
    alternate.push(Math.abs(array[0]));
    alternate.push(-1 * Math.abs(array[1]));
    return alternate.concat(alternateSign(array.slice(2)));
};
...
var numToText = function(str) {
    ...
    if (!isNaN(Number(words[0]))) {
        words[0] = numbers[Number(words[0])];
    } 
    let substr = words.slice(1).join(' ')
    if (substr.length < 1) {
        return words[0];
    } else {
        return words[0] + " " + numToText(substr);
    }
